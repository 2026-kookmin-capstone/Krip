...
fcm-token / mute 흐름은 RDB 만 터치 (실 Mongo 불필요). 알림창 (`notification` 컬렉션)
흐름은 실 Mongo 가 필요하므로 `mongo_db` / `notification_service` fixture 가 opt-in 으로
...
def _require_mongo_url() -> str:
    ...
    if not url:
        pytest.skip(
            "MONGODB_TEST_URL 환경변수가 설정되지 않아 알림창 통합 테스트를 건너뜁니다.",
...
def notification_service(mongo_db) -> NotificationService:
    """알림창 서비스 — stateless. 매 테스트마다 fresh 인스턴스로 충분."""
