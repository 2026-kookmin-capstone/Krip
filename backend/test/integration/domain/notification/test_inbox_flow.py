"""Notification 컬렉션 e2e 통합 테스트 (실 Mongo).

unit 테스트가 mock 으로 검증할 수 없는 영역을 cover:
    - `uq_notification_dedup` partial unique 인덱스가 실 Mongo 에 적용되어 동작
    - `display=true` 조건 partial filter 가 정확히 작동 (X 후 새 알림 가능)
...
검증 매트릭스:
    ...
    | 첫 알림 hide → 같은 키 새 insert        | 성공 (display=false 는 인덱스 밖)       |
...
from app.domain.notification.repository.notification import NotificationRepository
...
def _make_notification(
    *,
    ...
    """test 용 Notification 인스턴스 — 실 mongo 에 insert 가능한 minimal 형태."""
    return Notification(
        recipient_id=recipient_id,
        actor_id=actor_id,
        type=type,
        target_type=TargetType.FEED_POST if type != NotificationType.TRIPMATE_LIKE else TargetType.TRIPMATE_POST,
        target_id=target_id,
...
class TestPartialUniqueIndex:
    """`uq_notification_dedup` 인덱스가 실 mongo 에 적용되어 의도대로 동작하는지.

    좋아요 취소→재좋아요 시 알림 폭증 방지의 핵심 가드. unit 은 mock 이라 인덱스 자체를
    검증 못 함 — 통합으로만 보장 가능.
    """

    async def test_duplicate_active_notification_raises(self, mongo_db):
        """display=true 인 같은 (recipient, actor, type, target, null comment) 조합 → 충돌."""
        repo = NotificationRepository()

        await repo.insert(_make_notification())

        with pytest.raises(DuplicateKeyError):
            await repo.insert(_make_notification())

    async def test_hide_then_reinsert_succeeds(self, mongo_db, notification_service):
        """X 로 숨긴(display=false) 알림은 partial filter 밖 → 같은 키 새 insert 가능.

        Q1 결정: "X 후 같은 사람이 다시 좋아요 → 새 알림 OK". 실 인덱스로 보장.
        """
        repo = NotificationRepository()
        ...
        # X 버튼 — display=false
        await notification_service.hide_notification(
            recipient_id="USER_recipient",
            notification_id=str(first.id),
        )

        # 같은 키 새 알림 — partial filter 에서 첫 번째 빠짐 → 성공
    ...
    async def test_different_comment_ids_dont_conflict(self, mongo_db):
        """COMMENT 알림은 comment_id 가 매번 달라 자연 unique."""
        repo = NotificationRepository()

        await repo.insert(_make_notification(
            type=NotificationType.FEED_COMMENT, comment_id="CMT_1",
        ))
        # raise 없이 정상 — 다른 comment_id
        await repo.insert(_make_notification(
            type=NotificationType.FEED_COMMENT, comment_id="CMT_2",
        ))


# ──────────────────── list_notifications + 자동 읽음 처리 ────────────────────

class TestListNotificationsFlow:
    """알림창 e2e — 페이지네이션 + 자동 mark_as_read 의 atomic update."""

    async def test_list_returns_inserted_in_reverse_chronological(
        self, mongo_db, notification_service,
    ):
        ...
        assert result.notifications[0].target_id == "FDP_2"
        assert result.notifications[2].target_id == "FDP_0"
    ...
    async def test_mark_as_read_true_updates_db_atomically(
        ...
        repo = NotificationRepository()
        await repo.insert(_make_notification(target_id="FDP_1"))
        await repo.insert(_make_notification(target_id="FDP_2", actor_id="USER_b"))
...
class TestHideAtomic:
    ...
    async def test_hide_excludes_from_list(self, mongo_db, notification_service):
        repo = NotificationRepository()
    ...
    async def test_hide_other_user_raises_not_found(
        ...
        """타인 알림 hide 시도 → atomic query 매칭 실패 → NotFound 일원화."""
        repo = NotificationRepository()
        notif = _make_notification(recipient_id="USER_other")
        await repo.insert(notif)
    ...
    async def test_hide_invalid_objectid_raises_not_found(
        ...
        with pytest.raises(NotificationNotFoundError):
            await notification_service.hide_notification(
                recipient_id="USER_recipient",
                notification_id="not-an-objectid",
            )
...
class TestCountUnread:
    async def test_counts_only_unread_displayed(self, mongo_db, notification_service):
        repo = NotificationRepository()
        ...
        count = await notification_service.count_unread(recipient_id="USER_recipient")

        assert count == 3

    async def test_excludes_hidden_notifications(
        self, mongo_db, notification_service,
    ):
        """display=false (X 누름) 는 미읽음 카운트에서 제외."""
        repo = NotificationRepository()
        n1 = _make_notification(target_id="FDP_1")
        n2 = _make_notification(target_id="FDP_2", actor_id="USER_b")
        await repo.insert(n1)
        await repo.insert(n2)

        await notification_service.hide_notification(
            recipient_id="USER_recipient", notification_id=str(n1.id),
        )
...
class TestCascadeUserWithdrawn:
    """탈퇴 cascade — recipient 또는 actor 매칭 알림 모두 삭제."""
    ...
    async def test_deletes_recipient_and_actor_matching(
        ...
        deleted = await notification_service.cascade_user_withdrawn(user_id="USER_x")
