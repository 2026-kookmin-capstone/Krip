...
`create_comment` 호출 → RDB 댓글 INSERT → Mongo 알림 적재 흐름의 통합 검증.

검증 매트릭스:

    | 시나리오                          | RDB 댓글  | Mongo 알림 |
    |---|---|---|
    | 외부 actor 댓글                   | ✓         | ✓ 적재     |
    | 본인→본인 댓글                    | ✓         | ✗ skip     |
    | 같은 actor 가 여러 댓글 (다른 ID) | ✓ 여러 건 | ✓ 여러 건  |
    | delete_comment                    | RDB 삭제  | 알림 보존  |
    | 100자 초과 comment_preview        | -         | ellipsis   |
"""
import pytest

from app.domain.notification.model.notification import Notification, NotificationType
...
class TestCreateCommentFanout:
    """`create_comment` 가 RDB INSERT 후 Mongo 에 댓글 알림 적재."""

    async def test_external_actor_creates_notification_with_comment_id(
        ...
        assert doc["type"] == NotificationType.FEED_COMMENT.value
...
class TestMultipleCommentsCreateMultipleNotifications:
    """같은 actor 가 여러 댓글 — comment_id 가 매번 달라 알림도 매번 새로 적재.

    좋아요와 차이점: 좋아요는 lifetime unique, 댓글은 comment_id 마다 별개.
    """

    async def test_three_comments_create_three_notifications(
        self, mongo_db, feed_post_comment_service, seed_feed_post,
    ):
        post_id, owner_id = await seed_feed_post()
        actor_id = await _find_other_user(feed_post_comment_service.uow, owner_id)
...
# ──────────────────── delete_comment — 알림 보존 정책 ────────────────────

class TestDeleteCommentPreservesNotification:
    """댓글 삭제 시 알림 cascade 안 함 — 좋아요 취소 알림 보존 정책과 대칭."""

    async def test_delete_does_not_remove_notification(
        self, mongo_db, feed_post_comment_service, seed_feed_post,
    ):
        post_id, owner_id = await seed_feed_post()
        actor_id = await _find_other_user(feed_post_comment_service.uow, owner_id)
