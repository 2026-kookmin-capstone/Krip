...
def _patches(*, last_reads, baselines, residuals, lua_side_effect):
    ...
    ctx = [
        ...
        patch.object(rc, "_session_factory", lambda: _FakeSession()),
    ]
...
class TestRecoverUnread:
    async def test_passes_baseline_and_residual_to_lua_per_room(self):
        ...
        async def lua_stub(keys, args):
            _room, residual, _baseline, cap = args
            return min(residual, cap)  # 스텁: residual 반영
        ...
        assert calls["R1"] == ["R1", 5, 3, rc.UNREAD_COUNT_CAP]
        assert calls["R2"] == ["R2", 2, 0, rc.UNREAD_COUNT_CAP]
