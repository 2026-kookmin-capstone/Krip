...
경로 구조:
  ...
  uploads/perm/{user_id}/feed/{image_id}/{variant}.{ext}   - 피드 이미지 (다해상도 변형)
...
def feed_image_prefix(user_id: str, image_id: str) -> str:
    """피드 이미지 prefix — variant 별 키가 같은 디렉터리 하위에 모이도록 image_id 까지 포함."""
    return f"{user_id}/feed/{image_id}"
