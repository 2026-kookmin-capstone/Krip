"""피드 이미지 메인 테이블 (PostgreSQL).

기획안의 인스타그램 형 마이페이지 Feed 탭 — 유저별 이미지 갤러리.

설계 결정:
    - tripmate (post / post_image / post_like) 패턴을 차용한 RDB 모델. 좋아요/댓글이
      추가되면서 (image, user) 다대다 + (image 1:N user) 관계 표현이 RDB 에서 자연스럽다.
    - URL 3종 (original / small / medium) 모두 NOT NULL — 동기 업로드 (Pillow 처리 + S3 3건
      + INSERT) 가 한 트랜잭션이라 partial 상태가 DB 에 들어가지 않는다.
    - cascade: `user_id` FK ON DELETE CASCADE 로 탈퇴 시 자동 정리. 이미지 삭제 시
      `feed_image_like` / `feed_image_comment` 도 cascade 로 일괄 제거 (관계 cascade).
    - storage prefix: `uploads/perm/{user_id}/feed/{image_id}/{variant}.{ext}` — 단건 삭제
      = `delete_by_prefix({user_id}/feed/{image_id})` 한 호출로 변형 3종 모두 정리.
"""
...
from app.util.id_generator import generate_feed_image_id
...
class FeedImage(Base):
    ...
    인덱스 설계:
        `ix_feed_image_owner_visibility_created (user_id, visibility, created_at, image_id)`
    ...
    image_id = Column(String(50), primary_key=True, default=generate_feed_image_id)  # 피드 이미지 고유 ID
    ...
    caption = Column(String(CAPTION_MAX_LENGTH), nullable=True)  # 이미지 캡션 (최대 100자)
    ...
    user = relationship("User", backref="feed_images")
    likes = relationship(
        "FeedImageLike", back_populates="image", cascade="all, delete-orphan",
    )
    comments = relationship(
        "FeedImageComment", back_populates="image", cascade="all, delete-orphan",
    )

    __table_args__ = (
        Index(
            "ix_feed_image_owner_visibility_created",
            "user_id", "visibility", "created_at", "image_id",
        ),
    )

    def __repr__(self):
        return f"<FeedImage(image_id={self.image_id}, user_id={self.user_id}, visibility={self.visibility})>"
