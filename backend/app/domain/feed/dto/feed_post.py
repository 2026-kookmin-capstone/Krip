...
class FeedPostWithCounts:
    """FeedPost + 좋아요/댓글 수 합성 row — repository → service.

    repository 가 단일 SELECT (correlated subquery 2개) 로 한 번에 로드 → service 가
...
@dataclass
class FeedPostData:
    """피드 게시물 단건 응답 DTO — 좋아요/댓글 카운트 포함.

    카운트 값은 응답 시점의 스냅샷. 클라이언트가 좋아요/댓글 액션 후 게시물 상세를 다시
    조회하면 갱신된 값. 신규 업로드 직후는 항상 0.
    """
    post_id: str
    user_id: str
